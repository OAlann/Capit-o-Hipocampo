  ...
  run(client, message, args) {
    if (!args[0]) {
      ...
      const helpembed = new Discord.MessageEmbed()
        .setAuthor('📃 Lista de comandos PomoTrem')
        .addFields(categories)
        .setDescription(`Use ${process.env.PREFIX}ajuda + **nome do comando** para ver mais informações.`)
      ...
      if (!command) {
        const nocommandembed = new Discord.MessageEmbed()
          ...
          .setDescription(`Use \`${process.env.PREFIX}ajuda\` para listar todos os comandos ou \`${process.env.PREFIX}ajuda\` + *comando*.`)
          .setColor(process.env.COLOR)
